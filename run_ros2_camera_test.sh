...
docker run --rm -it \
  ...
  bash -lc '
    ...
    timeout 30s bash -lc "until ros2 topic echo /camera/image_raw --once >/dev/null 2>&1; do sleep 1; done" \
      || { echo "Camera frame did not arrive in time"; exit 1; }

    echo "Camera frame check passed."
    wait "$GZSERVER_PID"
  '
