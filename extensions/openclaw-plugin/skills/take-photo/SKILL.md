...
3. **Use savedPath when needed**: The tool also saves a local file by default and returns `savedPath` for file-based upload tools.
...
- By default, snapshots are saved under `/home/node/.openclaw/workspace/rosclaw_snapshots/` unless `savePath` is provided.
